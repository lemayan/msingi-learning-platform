...
// --- Learning outcome icons (matching the seeded icon string values) ---
const outcomeIconPaths: Record<string, React.ReactNode> = {
  layers: (
    <>
  ...
  workflow: (
    <>
      ...
      <path d="M17 3v2a2 2 0 0 1-2 2h-2" />
    </>
  ),
  gauge: (
    <>
...
/**
 * Renders a learning outcome icon by its string key (e.g. "layers", "rocket").
 * Falls back to a generic star icon if the key is unrecognised.
 */
export function OutcomeIcon({ name, className }: { name: string; className?: string }) {
  const paths = outcomeIconPaths[name]
  return (
    <svg
